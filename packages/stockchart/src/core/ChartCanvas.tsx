...
> {
  ...
  public handleZoom = (zoomDirection: any, mouseXY: any, e: any) => {
    ...
    const {zoomMultiplier = ChartCanvas.defaultProps.zoomMultiplier, zoomAnchor = ChartCanvas.defaultProps.zoomAnchor} =
      this.props;
      ...
      <ChartCanvasContext.Provider value={this.getContextValues()}>
        ...
        >
          <CanvasContainer
            ...
            width={width}
          />
          <svg className={className} height={height} style={{position: 'absolute', zIndex: zIndex + 5}} width={width}>
            {cursor}
            <defs>
              <clipPath id="chart-area-clip">
                <rect height={dimensions.height} width={dimensions.width} x="0" y="0" />
              </clipPath>
              {chartConfigs.map((each: any, idx: number) => (
                <clipPath key={idx} id={`chart-area-clip-${each.id}`}>
                  <rect height={each.height} width={each.width} x="0" y="0" />
                </clipPath>
              ))}
            </defs>
