...
import {isDefined, isNotDefined, noop} from 'src/core';
...
export class EquidistantChannel extends React.Component<EquidistantChannelProps, EquidistantChannelState> {
  ...
  public render() {
    ...
    const tempChannel =
      isDefined(current) && isDefined(current.endXY) ? (
        <EachEquidistantChannel interactive={false} {...current} appearance={appearance} hoverText={hoverText} />
      ) : null;

    return (
      <g>
        {channels.map((each, idx) => {
          const eachAppearance = isDefined(each.appearance) ? {...appearance, ...each.appearance} : appearance;
  ...
  private readonly handleDragChannelComplete = (e: React.MouseEvent, moreProps: any) => {
    ...
    if (isDefined(override)) {
      ...
      const newChannels = channels.map((each, idx) => (idx === index ? {...each, ...rest, selected: true} : each));
