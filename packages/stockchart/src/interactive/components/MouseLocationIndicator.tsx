import {functor, getClosestValue, getMouseCanvas, GenericChartComponent, isDefined, noop, shallowEqual} from 'src/core';
import * as React from 'react';
import {getXValue} from 'src/core/utils/ChartDataUtil';
...
export class MouseLocationIndicator extends React.Component<MouseLocationIndicatorProps> {
  ...
  private readonly xy = (e: React.MouseEvent, moreProps: any) => {
    ...
    if (enabled && isDefined(currentItem) && isDefined(e)) {
      const xValue =
        snap && !shouldDisableSnap(e) ? xAccessor(currentItem) : getXValue(xScale, xAccessor, mouseXY, plotData);
      const yValue =
