...
import {getClosestItemIndexes, getMouseCanvas, GenericChartComponent} from 'src/core';
...
function helper(props: any, moreProps: any) {
  ...
  const xSquareds = xs.map((x: any) => x ** 2);

  const b = (n * sum(xys) - sum(xs) * sum(ys)) / (n * sum(xSquareds) - sum(xs) ** 2);
